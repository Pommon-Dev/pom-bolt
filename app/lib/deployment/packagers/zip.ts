...
export class ZipPackager implements Packager {
  ...
  async package(files: Record<string, string>, options?: PackagerOptions): Promise<ArrayBuffer> {
    try {
      ...
      logger.debug(`Packaging ${Object.keys(filesToPackage).length} files into ZIP archive`);
      
      // Add files to the zip
      for (const [path, content] of Object.entries(filesToPackage)) {
        const fullPath = rootDir ? `${rootDir}/${path}` : path;
        logger.debug(`Adding file to ZIP: ${fullPath}`);
        zip.file(fullPath, content);
      }
      
      // Generate the zip file with settings optimized for Netlify
      // Netlify requires: 
      // - UNIX style paths (no backslashes)
      // - Properly set MIME types
      // - Reasonable compression level
      logger.debug('Generating ZIP with optimized settings for deployment services');
      const zipBlob = await zip.generateAsync({
        type: 'arraybuffer',
        platform: "UNIX",  // Explicitly set UNIX style paths
        compression: "DEFLATE", // Use DEFLATE compression
        compressionOptions: {
          level: 6 // Medium compression level (balance between size and speed)
        },
        comment: `Generated by PomBolt for deployment at ${new Date().toISOString()}` // Add comment for debugging
      });
      
      logger.debug(`Created ZIP archive with ${Object.keys(filesToPackage).length} files, size: ${zipBlob.byteLength} bytes`);
